...
def make_2x3_figure(save_path=None,  figsize=(6.5, 4.3), suptitle=None):
    cmap = 'inferno'
    boundaries = [0, 1, 3, 6, 20]
    norm = BoundaryNorm(boundaries, ncolors=256)
    fig, axes = plt.subplots(2, 3, figsize=figsize,sharex=True, sharey=True)


    D = [6.5,8.0]
    base_folder = ['Model3/det/p128','Model3/noise/p128','Model3/Bugs' ]

    # safe default N (used by the 'Bugs' branch); specific branches override this
    N = 1000
    i = 0
    for d in D:
        for base in base_folder:
            ...
            if base == base_folder[-1]:
                file_path = f'../Data/{base}/_N{N}_p{d:.2f}/dat1.h5'
                ...
                N = 10000
                file_path = f'../Data/{base}/_N{N}_p{d:.1f}/dat.h5'
                with h5py.File(file_path, 'r') as f:
    ...
    axes[0, 0].set_ylabel(f'$p={D[0]}$' '\n ' r'$y/R$', fontsize=13, labelpad=8)
    axes[1, 0].set_ylabel(f'$p={D[1]}$' '\n ' r'$y/R$', fontsize=13, labelpad=8)
    ...
    plt.subplots_adjust(hspace=0.09, wspace=0.2)
...
if __name__ == '__main__':
    ...
    out_path = os.path.join('..\..', 'Fig2.png')
